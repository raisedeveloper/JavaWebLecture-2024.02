...
public class Ex03_Calculator2 extends HttpServlet {
	...
	// <form action="/jw/ch06/calc" method="post"> 시작한 후에는 form 태그를 사용
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String num1_ = request.getParameter("num1");		//num1_ 은 name의 속성 
		String op = request.getParameter("op");
		String num2_ = request.getParameter("num2");
		int num1 = (num1_.equals("")) ? 0 : Integer.parseInt(num1_);
		int num2 = (num1_.equals("")) ? 0 : Integer.parseInt(num2_);
		int result = 0;
		switch (op) {
		case "+":
		...
		case "/":
			result = num1 / num2; break;
		default:
			result = -1; 		// 에러가 났다는 의미임
		}
		// 웹 페이지로 날려보냄
		// 첫 번째 방법 calc.jsp (servlet)을 직접 호출
		RequestDispatcher rd = request.getRequestDispatcher("/ch06/calc.jsp");
		request.setAttribute("num1", num1);
		request.setAttribute("num2", num2);
		request.setAttribute("result", result);
		...
		// redirection 이용 	Server -> Client -> Server
